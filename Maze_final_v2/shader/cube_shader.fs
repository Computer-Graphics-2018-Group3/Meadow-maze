...
void main() {
    ...
    vec3 diffuseColor = pow(texture(diffuseTexture, fs_in.TexCoords).rgb, vec3(gamma));
    ...
    vec3 result = (ambient + (1.0f-shadow)*(specular)) * texture(diffuseTexture, fs_in.TexCoords).rgb + (1.0f-shadow)*(diffuse);
