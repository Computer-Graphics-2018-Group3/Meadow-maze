...
void Cloth::update(float deltaTime) {
	...
	if (timeSinceLastChangeOfForce > 0.2f) {
		timeSinceLastChangeOfForce -= 0.2f;
		FORCE.x += 0.5f * ((float)rand()/(RAND_MAX+1.0f)*2 - 1);
		FORCE.z += 0.5f * ((float)rand()/(RAND_MAX+1.0f)*2 - 1);
	}
	...
	while (timeSinceLastUpdate > 0.01f) {
		...
		for (int i = 0; i < numBalls; ++i) {
			...
			if(currentBalls[i].fixed) {
				...
				nextBalls[i].velocity.x = (glm::clamp(nextBalls[i].velocity.x, 1.225f, 4.9f)+v) / 2.0f;
			}
		}
