...
int main()
{
	...
	// load models
    // -----------
    // Model ourModel("/Users/Eddie/Desktop/nanosuit/nanosuit.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// glm::vec3 pos = camera.Position + camera.Front*0.6f;
		// pos.y -= 0.55f;
		// characterModel = glm::translate(characterModel, pos); // translate it down so it's at the center of the scene
		// characterModel = glm::scale(characterModel, glm::vec3(0.04f, 0.04f, 0.04f)); // it's a bit too big for our scene, so scale it down
		// characterModel = glm::rotate(characterModel, glm::radians(camera.Yaw-90.0f), glm::vec3(0.0f, -1.0f, 0.0f));
		...
		glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
			...
			// depthShader.setMat4("model", characterModel);
		...
		// modelShader.setMat4("model", characterModel);
		...
		if (ImGui::CollapsingHeader("Do you need guide help")) {
			ImGui::RadioButton("Yes", &mazeMap, 0);
			ImGui::RadioButton("No", &mazeMap, 1);
		}
		ImGui::End();
		ImGui::Render();
