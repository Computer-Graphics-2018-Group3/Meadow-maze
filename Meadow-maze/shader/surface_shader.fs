...
void main()
{
	...
	// Height Color
	vec3 shallowColor = vec3(0.35, 0.64, 0.68);
	vec3 deepColor = vec3(0.03, 0.05, 0.10);
	
	float relativeHeight;	// from 0 to 1
	relativeHeight = (FragPos.y - heightMin + 0.3) / (heightMax - heightMin + 0.3);
	...
	// vec3 combinedColor = ambient + diffuse + reflectColor;
